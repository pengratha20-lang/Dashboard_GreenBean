...
def save_image_with_quality(image, filepath, quality=95):
    """
    Save image with appropriate quality settings based on format.
    
    Args:
        image: PIL Image object
        filepath: Path to save the image
        quality: Quality level for JPEG (1-100, default 95 for high quality)
    """
    try:
        # Get file extension
        ...
        # Ensure image is in RGB mode for JPEG (JPEG doesn't support transparency)
        if ext_lower in ['.jpg', '.jpeg'] and image.mode in ('RGBA', 'LA', 'P'):
            rgb_image = Image.new('RGB', image.size, (255, 255, 255))
            rgb_image.paste(image, mask=image.split()[-1] if image.mode == 'RGBA' else None)
            image = rgb_image
        
        # Save with quality for JPEG, without for others
...
def save_image(
    ...
    save_image_with_quality(img_with_watermark, original_path, quality=95)

    resized = image.copy()
    resized.thumbnail(resize_to)
    resized_with_watermark = add_watermark(resized, watermark_text)
    save_image_with_quality(resized_with_watermark, resized_path, quality=90)

    thumb = image.copy()
    thumb.thumbnail(thumb_size)
    thumb_with_watermark = add_watermark(thumb, watermark_text)
    save_image_with_quality(thumb_with_watermark, thumb_path, quality=85)
...
def save_image_organized(
    ...
    try:
        ...
        resized_filename = filename_with_timestamp  # Same filename, folder indicates version
        ...
        resized.thumbnail(resize_to)
        resized_with_watermark = add_watermark(resized, watermark_text)
        save_image_with_quality(resized_with_watermark, resized_path, quality=95)
        
        # Create and save thumbnail version
        thumb_filename = filename_with_timestamp  # Same filename, folder indicates version
        ...
        thumb.thumbnail(thumb_size)
        thumb_with_watermark = add_watermark(thumb, watermark_text)
        save_image_with_quality(thumb_with_watermark, thumb_path, quality=95)
